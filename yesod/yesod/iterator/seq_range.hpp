...
template <
	typename Sequence, typename NestedSequence,
	NestedSequence Sequence::value_type::*Nested
> struct seq_range_m {
	typedef Sequence container_type;
	typedef typename container_type::value_type contained_type;
	typedef NestedSequence nested_type;
	typedef typename nested_type::value_type value_type;
	typedef typename nested_type::reference reference;
	typedef typename nested_type::const_reference const_reference;
	typedef typename nested_type::size_type size_type;
	...
	private:
		...
		{
			++n_iter;
			while (n_iter == ((*c_iter).*Nested).end()) {
				++c_iter;
				if (c_iter == s->seq.end())
					return;
				n_iter = ((*c_iter).*Nested).begin();
			}
		}
		...
		void decrement()
		{
			if (c_iter == s->seq.end()) {
				--c_iter;
				n_iter = ((*c_iter).*Nested).end();
			}

			while (n_iter == ((*c_iter).*Nested).begin()) {
				if (c_iter == s->seq.begin())
					return;
				--c_iter;
				n_iter = ((*c_iter).*Nested).end();
		...
		) const
		{
			...
			rv += std::distance(
				((*x_iter).*Nested).begin(), other.n_iter
			);
			return rv;
		}
		...
		iterator_base(seq_range_m *s_, decltype(c_iter) c_iter_)
		: s(s_), c_iter(c_iter_)
		{}
	...
	seq_range_m(container_type &seq_)
	: seq(seq_)
	{}
	...
	{
		return iterator(
			this, seq.begin(), ((*seq.begin()).*Nested).begin()
		);
	}
