...
struct phaser {
	...
	)
	{
		...
		if (unarrived > 1)
			return root->internal_await_advance_default<false>(
				phase_out, alloc
			);
	...
	)
	{
		...
		auto phase_out(phase_type::make(s));
		if (phase_out == phase_in)
			return root->internal_await_advance_default<true>(
				phase_in, alloc
			);
		else
			return phase_out;
...
private:
		...
		while (true) {
			...
			if (parties || !unarrived) {
				if (!parent || (reconcile_state(s) == s)) {
					if (!unarrived) {
						root->internal_await_advance_default<false>(
							phase_out, alloc
						);
						s = state.load();
	...
	template <bool Interruptible, typename Allocator>
		...
		return internal_await_advance<false>(
			phase_in, [this, &alloc](phase_type phase_) {
				return wait_node<Allocator>::create(
					this, phase_, Interruptible, alloc
				);
			}
		);
	}
	...
	)
	{
		return internal_await_advance<true>(
			phase_in, [this, rel_time, &alloc](phase_type phase_) {
				return timed_wait_node<Allocator>::create(
					this, phase_, true, rel_time, alloc
				);
			}
		);
	}
