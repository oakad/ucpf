...
#if !defined(UCPF_YESOD_TEST_COMMON_OCT_16_2013_1520)
#define UCPF_YESOD_TEST_COMMON_OCT_16_2013_1520
...
struct queue_stress_tester {
	...
	void run(queue & stk)
	{
		...
		for (int i = 0; i != reader_threads; ++i)
			reader.create_thread(std::bind(
				&queue_stress_tester::template get_items<queue>,
				this, std::ref(stk)
			));

		for (int i = 0; i != writer_threads; ++i)
			writer.create_thread(std::bind(
				&queue_stress_tester::template add_items<queue>,
				this, std::ref(stk)
			));
