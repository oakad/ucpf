...
#include <mina/detail/from_ascii_decimal_i.hpp>
...
struct from_ascii_decimal_dispatch {
	template <
...
template <typename T>
struct from_ascii_decimal_dispatch<T, false, true> {
	template <
		typename FirstIterator, typename LastIterator, typename Alloc
	> static bool apply(
		T &v, FirstIterator &&first, LastIterator const &last,
...
template <typename T>
struct from_ascii_decimal_dispatch<T, true, true> {
	template <
		typename FirstIterator, typename LastIterator, typename Alloc
	> static bool apply(
		T &v, FirstIterator &&first, LastIterator const &last,
...
{
	return detail::from_ascii_decimal_dispatch<
